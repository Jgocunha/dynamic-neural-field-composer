...
| `Invalid simulation file: "formatVersion" must be a whole number >= 0: <path>` | `"formatVersion"` is present but isn't a non-negative integer (a string, a float like `1.5`, or a negative). | Load aborts, simulation unchanged — `identifier`/`deltaT` are not applied. |
| `Simulation file <path> declares format version <N>, but this build of dnf-composer only knows version <M> - this file was written by a newer version of dnf-composer. Loading it anyway; ...` | The file's `formatVersion` is higher than this build's `kCurrentFormatVersion`. | **WARNING, not an error.** The load proceeds best-effort; anything the newer format added is ignored by the tolerant paths below (unrecognized labels, unresolvable interactions). Update dnf-composer if elements are missing. See [.dnf File Schema → Schema version](DNF-File-Schema#schema-version). |
| `Invalid simulation file: could not build the elements of <path> (<exception text>) - load aborted.` | Either an element passed the up-front checks but its constructor still rejected it (typically a contract the pre-check does not re-derive, such as the samples-per-axis ceiling reached from a valid `x_max` with a very small `d_x`), or the element object is missing a required, type-specific field (e.g. `tau`, `amplitude`, `width` — see [.dnf File Schema](DNF-File-Schema#element-specific-fields-by-label)), which surfaces as a `json.exception.out_of_range` in `<exception text>`. | Load aborts and is undone: elements this load added are removed, and the `identifier` and `deltaT` it had already read from the file are restored to their previous values. Elements the simulation held **before** the load are left untouched, since loading appends rather than replaces. |
