...
| `Invalid simulation file: could not build the elements of <path> (<exception text>) - load aborted.` | An element passed the up-front checks but its constructor still rejected it — typically a contract the pre-check does not re-derive, such as the samples-per-axis ceiling reached from a valid `x_max` with a very small `d_x`. | Load aborts and is undone: elements this load added are removed, and the `identifier` and `deltaT` it had already read from the file are restored to their previous values. Elements the simulation held **before** the load are left untouched, since loading appends rather than replaces. |
