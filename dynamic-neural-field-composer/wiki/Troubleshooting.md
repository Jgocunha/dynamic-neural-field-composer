...
### `ERROR`: "Input '...' has a different dimensionality than '...'."

Logged (not thrown) from `Element::addInput()` at `ERROR` level when you connect a 1D element to a 2D element (or vice versa) directly — the connection is silently rejected. Before this check existed, a 1D element and a 2D element whose *flattened* sizes happened to match (e.g. a 1D field of size 10 and a 2D field of 5×2) would connect without complaint and produce silently incorrect dynamics, since the underlying data has a different spatial layout in each case.

This check only applies to elements that connect their own "input" directly to their own shape (the common case). `Resize`/`Resize2D`/`Collapse`/`Expand`/`FieldCoupling` are exempt — they exist specifically to bridge different shapes/dimensionalities and validate the connection themselves.
