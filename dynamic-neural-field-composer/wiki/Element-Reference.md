...
A resolution bridge that resamples a 1D signal from one spatial size to another. Use it whenever two elements that need to be connected operate at different spatial resolutions — for example, a high-resolution perceptual field feeding a lower-resolution decision field.

`Resize` stores the **input** size in its common parameters so the framework's connection-size guard accepts upstream elements of that size. The **output** is an independently sized buffer populated each `step()` call using the chosen interpolation algorithm.
...
| Parameter | Default | Description |
|---|---|---|
| `outputSize` | `100` | Number of discrete positions in the output vector |
| `outputStep` | `1.0` | Spatial step size associated with the output field |
| `interpolation` | `LINEAR` | Resampling algorithm (see table below) |

### Interpolation methods

| Enum value | Description |
|---|---|
| `ResizeInterpolation::NEAREST` | Nearest-neighbour — picks the closest input sample; fastest, may introduce staircase artefacts |
| `ResizeInterpolation::LINEAR` | Linear interpolation between the two bounding input samples; smooth and suitable for most cases |
| `ResizeInterpolation::CUBIC` | Catmull-Rom cubic spline — highest quality, especially when up-sampling |

### Components

| Name | Description |
|---|---|
| `"input"` | Summed input from connected upstream elements (size = common parameter size) |
| `"output"` | Resampled signal (size = `outputSize`) |

### Usage

```cpp
// Downsample: field of 200 positions → 100 positions
...
resize->addInput(field200, "output");   // upstream output size must match 200
field100->addInput(resize, "output");   // resize output size matches field100 (100)
```

### Usage notes

- The **input** size (set by `ElementCommonParameters`) and the **output** size (`outputSize`) are independent. The `addInput()` size guard checks against the input size, not the output size.
- `setParameters()` calls `init()` internally, immediately resizing the output buffer.
- When `outputSize` equals the input size, the element copies the signal unchanged (no resampling work is done).
