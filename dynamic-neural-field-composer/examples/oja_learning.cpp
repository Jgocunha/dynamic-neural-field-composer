...
int main()
{
	try
	{
		...
		const auto simulation = std::make_shared<Simulation>("Oja learning (example)",
			1.0, 0.0, 0.0);
		...
		const auto fc_1 = factory.createElement(element::FIELD_COUPLING,
			element::ElementCommonParameters{element::ElementIdentifiers{"coupling source-output oja"}, output_dimensions},
		...
		// Oja's rule is Hebbian learning with an added weight-decay term that keeps
		// the weight vector normalized, avoiding the unbounded growth Hebbian learning suffers from.
		constexpr auto rule = LearningRule::OJA;
