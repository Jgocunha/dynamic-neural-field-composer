...
// the flattened (row-major, 2500-value) activation profiles to data/dnfc/ as CSV.
//
// getComponent("neural field u","activation") already returns the row-major
// flattened std::vector<double> for a 2D field, so the save path is identical to
// 1D; the only 2D-specific change is zeroing GaussStimulus2D elements in phase 2.
//
...
//
// Usage: dnfc_suite_runner_2d <simulations_dir> <output_dir>
...
// Set amplitude=0 for all 2D stimulus elements in the simulation.
// Stimulus unique names: "gauss stimulus 2d" (single) or "gauss stimulus 2d N" (multi).
static void zero_all_stimuli(const std::shared_ptr<Simulation>& sim)
{
    for (auto& elem : sim->getElements()) {
        auto stim = std::dynamic_pointer_cast<GaussStimulus2D>(elem);
        if (!stim) continue;
...
int main(int argc, char* argv[])
{
    const fs::path sim_dir = (argc >= 2)
        ? fs::path(argv[1])
        : fs::path(R"(C:\dev-files\dynamic-field-theory-software\cross-platform-validation-2d\simulations\dnfc)");

    const fs::path out_dir = (argc >= 3)
        ? fs::path(argv[2])
        : fs::path(R"(C:\dev-files\dynamic-field-theory-software\cross-platform-validation-2d\data\dnfc)");
